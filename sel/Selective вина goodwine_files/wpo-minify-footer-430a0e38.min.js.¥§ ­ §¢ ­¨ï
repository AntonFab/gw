$(function(){$(document).on("click",".js-selective-wine-tags .js-tag-counter",function(){$(this).siblings().css("display","block");$(this).hide()});$(window).resize(function(){});function updateTagCount(){$(".js-selective-wine-tags .js-tag-counter").hide();$(".js-selective-wine-tags .selective-wine-tags__item:not(.js-tag-counter)").css("display","block");let tags=$(".js-selective-wine-tags .selective-wine-tags__item:not(.js-tag-counter)");let showblocks=[];let hideBlocks=[];let widthBlock=parseInt($(".js-selective-wine-tags").css("width"));let maxRow=$(".js-selective-wine-tags").data("rowsToShow");let containerWidth=widthBlock;tags.each(function(index,element){let elementWidth=$(element).outerWidth(!0);if(containerWidth-elementWidth<0){containerWidth=widthBlock;maxRow--}
if(maxRow>0){containerWidth=containerWidth-elementWidth;showblocks.push(element)}else{hideBlocks.push(element);$(element).css("display","none")}});if(hideBlocks.length>0){$(showblocks[showblocks.length-1]).css("display","none");$(".js-selective-wine-tags .js-tag-counter").show();$(".js-selective-wine-tags .js-tag-counter span").text("+ "+(hideBlocks.length+1))}}
$(window).resize(function(){updateTagCount()});updateTagCount();let click=!1;$(document).on("click",".selective-wine-catalog__load-more",function(){let ajaxurl=gwArgs.ajaxurl;if(ajaxurl!=""){if(!click){click=!0;let $exclude=Array();let sort_tag=$(".selective-wine-grid").data("tag");$(".selective-wine-card").each(function(i,el){$exclude.push($(el).data("id"))});setPreloader($(this).parent());$(this).hide();$.ajax({url:ajaxurl,data:{action:"get-more-selective-wine",exclude:$exclude,sort_tag:sort_tag,},type:"POST",success:function(response){let itemsList=$(response).children();let appendList=[];let showBtn=!1;if(itemsList.length>6){showBtn=!0;itemsList.each(function(i,el){if(i<6){appendList.push(el)}})}else{appendList=$(response).children()}
$(".selective-wine-grid").append(appendList);if(showBtn){click=!1;$(".selective-wine-catalog__load-more").show()}
removePreloader($(".selective-wine-catalog__load-more").parent())},error:function(e){console.log("Error!",e);click=!1},})}}
return!1});let $element=$(".selective-wine-grid");$(window).scroll(function(){let bottom=$element.position().top+$element.outerHeight(!0);let windowBotom=$(window).height()+$(window).scrollTop();if(bottom<windowBotom){getMoreItems()}});let loadActive=!1;function getMoreItems(){let ajaxurl=gwArgs.ajaxurl;if(ajaxurl!=""){if(!loadActive){loadActive=!0;let $exclude=Array();let sort_tag=$(".selective-wine-grid").data("tag");$(".selective-wine-card").each(function(i,el){$exclude.push($(el).data("id"))});setPreloader($(".selective-wine-grid"));$.ajax({url:ajaxurl,data:{action:"get-more-selective-wine",exclude:$exclude,sort_tag:sort_tag,},type:"POST",success:function(response){let itemsList=$(response).children();let appendList=[];loadActive=!0;if(itemsList.length>6){loadActive=!1;itemsList.each(function(i,el){if(i<6){appendList.push(el)}})}else{appendList=$(response).children()}
$(".selective-wine-grid").append(appendList);removePreloader($(".selective-wine-grid"))},error:function(e){loadActive=!1;console.log("Error!",e)},})}}}});
